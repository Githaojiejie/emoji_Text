...
namespace EmojiUI
{
	public class InlineManager : MonoBehaviour
	{
		...
		private readonly Dictionary<string, SpriteInfoGroup> _alltags = new Dictionary<string, SpriteInfoGroup>(CustomOrdinalStringComparer.GetComparer());

		private readonly Dictionary<string, KeyValuePair<SpriteAsset, SpriteInfoGroup>> _spritemap = new Dictionary<string, KeyValuePair<SpriteAsset, SpriteInfoGroup>>(CustomOrdinalStringComparer.GetComparer());
...
//				}
//			}
//			Debug.Log("find :{0} atlas resource", result.Length);
...
//					Debug.LogError("not prepred atlasAsset named :{0}", asset);
//				}
//			}
//#endif
			if (_render != null)
		...
		void RebuildTagList()
		{
			...
			for (int i = 0; i < _sharedAtlases.Count; ++i)
			{
				...
				for (int j = 0; j < asset.listSpriteGroup.Count; ++j)
				{
					...
					if (_alltags.TryGetValue(infogroup.tag, out group))
					{
						Debug.LogError($"already exist :{infogroup.tag} ");
					}
        ...
        public List<SpriteAsset> GetAllAtlases()
        {
            return _sharedAtlases;
        }
