		...
		void FillSpriteTag(InlineText text,StringBuilder stringBuilder, Match match,int matchindex, int atlasId,string atlasTag)
		{
			...
			if (text.Manager != null && text.Manager.CanRendering(atlasId) && text.Manager.CanRendering(atlasTag))
			{
				...
				if (tagSprites != null && tagSprites.spritegroups.Count > 0)
				{
                    ...
                    float autosize = tagSprites.size; //Mathf.Min(tagSprites.size, Mathf.Max(spacewid, spaceheight));
                    ...
                    int tempIndex = stringBuilder.Length * 4;

                    stringBuilder.Append(@"<quad size=" + (autosize - (autosize - spaceheight) / 2) + " width=" + 0.001 + " />");
                    ...
                    if (fillspacecnt > 0)
					{
						...
						tempSpriteTag.FillIdxAndPlaceHolder(tempIndex - 1,matchindex,fillspacecnt);
